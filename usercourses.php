...
<?php
    ...
    // SQL Query to Fetch Course Data with Faculty Name
    $query = "
        SELECT c.CourseIDNumber, c.CourseName, c.CourseDescription, 
               CONCAT(f.FName, ' ', LEFT(f.MI, 1), '. ', f.LName) AS FacultyName
        FROM course c
        LEFT JOIN faculty f ON c.FacultyID = f.FacultyID"; // Ensure correct table and field names
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                echo "<tr>
                        ...
                        <td>" . htmlspecialchars($row['FacultyName'] ?? 'N/A') . "</td>
                      </tr>";
            }
            echo "</tbody></table>";
        } else {
...
?>
  ...
  <script>
    ...
    $('#coursesTable').DataTable(); // Correct ID
});
