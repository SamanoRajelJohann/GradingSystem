...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] === 'add') {
    $student_id_number = mysqli_real_escape_string($mysqli, $_POST['student_id_number']);
    $last_name = mysqli_real_escape_string($mysqli, $_POST['last_name']);
    $first_name = mysqli_real_escape_string($mysqli, $_POST['first_name']);
    $middle_initial = mysqli_real_escape_string($mysqli, $_POST['middle_initial']);

    // Insert query
    $sql = "INSERT INTO student (StudentIDNumber, LName, FName, MI) 
            VALUES ('$student_id_number', '$last_name', '$first_name', '$middle_initial')";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] === 'edit') {
    ...
    $check_query = "SELECT StudentIDNumber, LName, FName, MI FROM student WHERE StudentID = ?";
    ...
    $has_changes = false;
    if ($current_values['StudentIDNumber'] != $student_id_number ||
    ...
    if ($has_changes) {
        // Update the student
        $update_query = "UPDATE student SET StudentIDNumber = ?, LName = ?, FName = ?, MI = ? WHERE StudentID = ?";
        $update_stmt = $mysqli->prepare($update_query);
        $update_stmt->bind_param("isssi", $student_id_number, $last_name, $first_name, $middle_initial, $student_id);
        ...
        if ($update_stmt->execute()) {
            ...
            echo "<script>
                alert('Error updating student: " . $update_stmt->error . "');
            </script>";
        }
        $update_stmt->close();
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete') {
    if (isset($_POST['student_id'])) {
        ...
        // First, insert into deleted_student table
        $insert_sql = "INSERT INTO deleted_student (StudentID, StudentIDNumber, LName, FName, MI)
                      SELECT StudentID, StudentIDNumber, LName, FName, MI
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->execute()) {
                ...
                echo "<script>alert('Error deleting student: " . $mysqli->error . "');</script>";
            }
        } else {
...
<body>
  ...
  <div class="container mt-4">
    <button type="button" class="btn btn-primary" data-bs-toggle="modal" style="width:175px;" data-bs-target="#addStudentModal">
      Add New Student
    </button>
      ...
      <form method="POST" action="">
        ...
        <div class="modal-body">
          <div class="mb-3">
            ...
            <input type="number" class="form-control" id="student_id_number" name="student_id_number" required>
          </div>
          <div class="mb-3">
...
<div class="modal fade" id="editStudentModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="edit_student_id_number" class="form-label">Student ID Number</label>
                        <input type="number" class="form-control" id="edit_student_id_number" name="student_id_number" required>
                    </div>
                    <div class="mb-3">
  ...
  <div class="container mt-4">
    ...
    $query = "SELECT StudentID, StudentIDNumber, LName, FName, MI FROM student";
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                echo "<tr>
                        ...
                        <td>
                            <button class='btn btn-primary btn-sm' onclick='editStudent(" . $row['StudentID'] . ", \"" . htmlspecialchars($row['StudentIDNumber']) . "\", \"" . htmlspecialchars($row['LName']) . "\", \"" . htmlspecialchars($row['FName']) . "\", \"" . htmlspecialchars($row['MI']) . "\")'>Edit</button>
  ...
  <script>
    ...
    function editStudent(studentId, studentIdNumber, lastName, firstName, middleInitial) {
        document.getElementById('edit-student-id').value = studentId;
        document.getElementById('edit_student_id_number').value = studentIdNumber;
